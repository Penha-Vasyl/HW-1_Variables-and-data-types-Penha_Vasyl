...
const payment = 500;
const restMoney = Number((payment - price).toFixed(2));
console.log(restMoney);

// Виведіть середнє значення цін, округлене до другого знаку після коми
const averagePrice = Number(((headphoneSven + headphoneKoss + headphoneSennheiser)/3).toFixed(2));
...
const amountForPayment = Number((price - (price * (discount / 100))).toFixed(2));
console.log('Сума до оплати = ',amountForPayment);
const profit = Number((amountForPayment - price / 2).toFixed(2));
...
Округлена вартість товарів = ${priceRound}
